...
use ratatui::widgets::{Block, ListItem, Paragraph, Borders};

const DEFAULT_BLOCK: ratatui::widgets::Block =ratatui::widgets::Block::bordered().border_type(ratatui::widgets::BorderType::Rounded);
...
pub fn render(app: &App, frame: &mut ratatui::Frame) {
    ...
    let list_items = bookmarks_vec
        .iter()
        .map(|bookmark| bookmark.to_list_item(app, bookmarks_vec.iter().position(|b| b == bookmark).unwrap()))
    ...
    frame.render_widget(&search_bar, layout_areas.search_area.inner(Margin::new(3, 1)));
...
fn get_layout_areas(frame: &ratatui::Frame) -> LayoutAreas {
    ...
    let vertical_flex =
        Layout::vertical([Constraint::Max(3), Constraint::Max(3), Constraint::Fill(1), Constraint::Max(1)])
            .flex(Flex::Center);
...
fn render_preview(app: &App, total_preview_area: ratatui::layout::Rect, frame: &mut ratatui::Frame) {
    ...
    // Render the title
    let bookmark_name = app.selected_bookmark.clone().unwrap_or(Bookmark::default()).name.clone().unwrap_or("".to_string());
...
fn get_bookmark_preview(app: &App) -> ratatui::widgets::Paragraph {
    ...
    if app.selected_bookmark.clone().is_none(){
        return ratatui::widgets::Paragraph::new("No bookmark selected").style(Style::default().fg(Color::LightBlue)).centered();
    }

    let bookmark_path_string = app.selected_bookmark.clone().unwrap_or(Bookmark::default()).path.clone().unwrap_or("".to_string());

    let bookmark_path = std::path::Path::new(&bookmark_path_string);
    if !bookmark_path.exists() {
        return ratatui::widgets::Paragraph::new("Path does not exist").style(Style::default().fg(Color::Red)).centered();
    }
    if bookmark_path.is_file() {
        return ratatui::widgets::Paragraph::new("File preview not yet supported").style(Style::default().fg(Color::Blue)).centered();
    }
    ...
    // Add the block style
    preview.block(Block::default().style(Style::default().fg(Color::White)).borders(Borders::TOP))
...
fn get_bookmark_directory_preview(app: &App) -> ratatui::widgets::Paragraph {
    ...
    let directory_contents_lines = filtered_directory_contents
        .iter()
        .map(|entry|
            get_preview_line(entry.to_owned())
        )
        .collect::<Vec<Line>>();
...
fn get_preview_line(entry: DirectoryPreviewEntry) -> Line<'static> {
    let icon = if entry.is_file { "" } else if entry.is_dir { "" } else { "" };

    let tree_symbol = if entry.is_last { "└─" } else { "├─" };

    let style = if entry.is_dir { Style::default().fg(Color::Blue) } else { Style::default().fg(Color::White) };

    return Line::from(vec![Span::styled(tree_symbol, Style::default().fg(Color::DarkGray)),Span::styled(format!(" {} {}", icon, entry.name), style)]);
}

fn get_path_line(path_string: String) -> Line<'static> {

    return Line::from(vec![Span::styled(format!("{}", path_string), Style::default().fg(Color::DarkGray))]);
}
fn get_preview_areas(preview_area: ratatui::layout::Rect) -> (ratatui::layout::Rect, ratatui::layout::Rect) {
    let main_area = preview_area;
    let vertical_flex =
        Layout::vertical([Constraint::Max(2), Constraint::Fill(1)])
            .flex(Flex::Center);

    let [title_area, preview_area] =
        vertical_flex.areas(main_area);


    return (title_area.offset(Offset { x: 0, y: 1 }), preview_area.inner(Margin::new(1, 1)))
}
...
impl Bookmark {
    fn to_list_item(&self, app: &App, list_item_index: usize) -> ListItem<'_> {
        ...
        let is_directory = std::path::Path::new(&self.path.clone().unwrap_or("".to_string())).is_dir();
        ...
        let bookmark_name_span = if is_selected {
            Span::styled(bookmark_name.clone(), Style::default().fg(Color::Green).underlined())
        } else {
        ...
        let bookmark_description_space = if self.name.clone().unwrap_or("".to_string()).len() < BOOKMARK_TITLE_WIDTH {" ".repeat(BOOKMARK_TITLE_WIDTH - bookmark_name.len() )} else {"".to_string()};
        let bookmark_description_string = format!("{}{}",bookmark_description_space,self.description.clone().unwrap_or("".to_string()));
        let bookmark_path_span = Span::styled( bookmark_description_string, Style::default().fg(Color::DarkGray));
        
        let line_content = vec![decoration_span,icon_span, bookmark_name_span, bookmark_path_span];
